...
# VMI-Jokon — Prompt de cadrage Tree-of-Thought · Gemini Deep Research + NotebookLM → Codex
...
| Entité / `entity_id` | **VMI-Jokon** / `vmi-jokon` |

| Rang dans la séquence du dépôt | 5 / 11 |
...
Point d'attention : les limites de longueur des champs de NotebookLM évoluent ; le BLOC B et les requêtes C sont volontairement courts, la méthode détaillée étant portée par la source `00_BRIEF_vmi-jokon`.
...
**Ta mission dans ce programme : la recherche de cadrage de l'entité VMI-Jokon uniquement.** Tu produis une carte de recherche (arbre Tree-of-Thought exploré, élagué et documenté) et un registre de preuves. Tu ne produis **ni** diagnostic final, **ni** score de maturité, **ni** ROI, **ni** recommandation fournisseur. Codex partira de ton livrable comme point de départ et comme mind map de recherche.
...
Règle de classement Core / Enabler / Support : **Core** = transforme ou vend le produit / service ; **Enabler** = coordonne les flux ; **Support** = administre. Chaîne de Porter de référence : principales = logistique amont, opérations, logistique aval, marketing & ventes, services ; soutien = infrastructure (direction, finance, juridique), GRH, développement technologique (R&D, ingénierie, SI/data), approvisionnements. Deux chaînes distinctes à documenter séparément puis à relier : VMI (découpage/emboutissage grande série de contacts et pièces de précision) et Jokon (conception et fabrication de feux et signalisation).

### A.3 Fiche entité — VMI-Jokon

**Désambiguïsation :** VMI-Jokon = VMI (vmi-31.com, Villemur-sur-Tarn) + Jokon (jokon.de, Bonn et Folschviller) ; PAS VMI Holland / TKH / vmi-group.com, PAS les autres « VMI » (vendor managed inventory).

**Identité (faits d'amorce à revérifier).** Groupe industriel franco-allemand acquis par Hivest le **30/07/2026** (LBO majoritaire ; Marc Laisné conserve près de 40 % du capital avec le management ; conseil vendeurs : Financière de Courcelles).

- Près de 60 M€ de CA consolidé 2025 ; environ 270 personnes, principalement en France et en Allemagne.

- **VMI** (Villemur-sur-Tarn, Haute-Garonne) : connecteurs électriques et pièces métalliques de précision pour l'automobile ; plus de 350 millions de pièces par an livrées aux câbleurs et équipementiers de rang 1.

- **Jokon** (Bonn, DE ; Folschviller, Moselle) : éclairage et signalisation pour camping-cars, caravanes, bus, remorques, machinisme agricole ; plus de 600 clients ; près de 3 000 références.

- Thèse Hivest : nouveaux produits, montée en gamme, croissance externe accélérée en Europe et en Amérique du Nord ; plusieurs acquisitions à l'étude.

- Direction : Romain Bontemps (MD VMI) ; Martin Zavelberg (MD Jokon) ; **Ludovic Weiss, COO de Jokon** (périmètre filiale, pas groupe — déjà au registre du dépôt).

**Pièges de périmètre :** rôle exact de Marc Laisné (président ? fondateur ?) non établi ; données VMI et Jokon à ne pas additionner sans consolidation ; deal post-cutoff proche (juillet 2026) → peu de sources.
...
### A.5 Arbre Tree-of-Thought de VMI-Jokon

**Racine R0 —** Où VMI-Jokon crée-t-il sa marge entre pièces de précision grande série (VMI) et systèmes d'éclairage/signalisation à forte variété (Jokon), et quelles couches data/IA soutiennent la montée en gamme et le build-up Europe / Amérique du Nord annoncés ?
...
| H1.1 | Le groupe a une holding de tête nommée et des entités FR/DE distinctes | Registres Pappers / Handelsregister | `VMI Villemur-sur-Tarn Pappers ; Jokon GmbH Handelsregister Bonn` |

| H1.2 | Des acquisitions ont suivi le closing de juillet 2026 | Communiqués post-cutoff (marquer post_cutoff) | `VMI-Jokon acquisition 2026` |
...
| H2.1 | La répartition du CA VMI / Jokon est publique | Comptes sociaux FR, Bundesanzeiger DE | `Jokon GmbH Jahresabschluss Bundesanzeiger ; VMI Villemur comptes` |

| H2.2 | Le prix du cuivre/laiton pèse sur la marge VMI (répercussion contractuelle ?) | Mentions d'indexation, **sector** | `stamped terminals copper price pass-through automotive supplier` |
...
| H3.1 | VMI est exposé à la baisse des volumes automobiles européens et à la transition VE | Presse, clients rang 1 | `VMI Villemur connecteurs automobile véhicule électrique` |

| H3.2 | Jokon est un acteur de niche face à Aspöck, Hella (Forvia), Vignal, Fristom, WAS | Présence salons (Caravan Salon), catalogues concurrents | `Jokon Aspöck Vignal trailer lighting market ; Caravan Salon Jokon 2025` |
...
| H4.1 | VMI : la maîtrise des outils de découpe (conception outillage, cadence, maintenance presses) est le cœur | Pages savoir-faire, offres d'emploi outilleur | `VMI Villemur outillage découpe presse offre emploi` |

| H4.2 | Jokon : la gestion de 3 000 références et 600 clients (catalogue, variantes, homologations) est le cœur | Catalogue en ligne, homologations ECE | `Jokon Katalog ECE R48 Homologation Leuchten` |
...
| H5.1 | Un COO groupe sera nommé (ou le COO Jokon étend son périmètre) | Communiqués, LinkedIn public | `"VMI-Jokon" COO ; "Ludovic Weiss" Jokon` |

| H5.2 | Aucune DSI groupe ; IT par site | Offres IT, profils | `Jokon IT-Leiter ; VMI Villemur informatique responsable` |

| H5.g | Un responsable data / BI / IA / ML (interne ou partenaire) est identifiable | Offres « data », profils publics, prestataires nommant l'entreprise | `"VMI-Jokon" data OR BI OR "machine learning" OR IA` |
...
| H6.1 | Machines : ERP différents entre VMI et Jokon (sujet d'intégration) | Offres citant l'ERP, références éditeurs | `Jokon ERP SAP OR proALPHA OR abas ; VMI Villemur ERP` |

| H6.2 | Mesure : contrôle qualité automatisé sur grande série (vision, SPC) chez VMI | Certifications IATF, offres qualité | `VMI Villemur IATF 16949 contrôle vision` |

| H6.3 | Matière : données catalogue/variantes Jokon structurées (PIM, configurateur) | Site catalogue, e-commerce B2B | `Jokon Online-Katalog Konfigurator` |

| H6.g | La stratégie ML **observée** est publiquement établie (modèles en production, MLOps, budget, owner) | Toute preuve de production ; sinon `unknown`, pas zéro | `"VMI-Jokon" intelligence artificielle OR "machine learning" OR IA production` |
...
| H7.1 | La réglementation d'éclairage (ECE R148/R149/R48) et la LED renouvellent la gamme Jokon | Textes UNECE + communications Jokon | `UNECE R148 R149 trailer lamps transition ; Jokon LED neue Produkte` |

| H7.2 | Le marché camping-car/caravane est cyclique (baisse post-2022) | Statistiques CIVD / ECF **(sector)** | `CIVD Zulassungen Reisemobile 2025 2026` |
...
| H8.1 | Maintenance prédictive presses + vision grande série sont des patterns matures | Cas **sector** (stamping) | `stamping press predictive maintenance AI case ; vision inspection terminals` |

| H8.2 | Prévision de demande saisonnière et configuration de gamme pour Jokon | Cas **sector** (aftermarket automotive lighting) | `aftermarket demand forecasting seasonal RV parts machine learning` |
...
Rôle : analyste de recherche du programme « Hivest – Portfolio AI Diagnostic », entité VMI-Jokon (entity_id vmi-jokon), cutoff 23/09/2026.

Tu réponds UNIQUEMENT à partir des sources du notebook. La source « 00_BRIEF_vmi-jokon » fixe la méthode (arbre Tree-of-Thought B1–B8, Ishikawa 6M, contrat de page exec v0.3, format HANDOFF_CODEX) ; toutes les autres sources sont des preuves.
...
VMI-Jokon (vmi-jokon) — Applique B2 et B3 du brief 00_BRIEF_vmi-jokon. Donne CA, EBITDA, effectifs, BFR, capex, dette par année et périmètre, divergences exposées. Puis segments (produit / canal / pays), 4P, cinq forces de Porter par segment, pairs nationaux et internationaux avec leur actionnariat. Aucune matrice BCG si croissance de marché externe et part relative absentes. Statut et source sur chaque ligne.
...
VMI-Jokon (vmi-jokon) — Applique la section BLOC D du brief 00_BRIEF_vmi-jokon. Réévalue chaque pensée H (V×P÷C, statut final), élague, liste les questions d'entretien. Puis produis UN SEUL bloc YAML « HANDOFF_CODEX » strictement conforme au schéma du brief, suivi d'un bloc Mermaid « mindmap » de l'arbre exploré (racine R0, branches B1–B8, pensées avec statut). Aucune information absente des sources.
...
  root: "Où VMI-Jokon crée-t-il sa marge entre pièces de précision grande série (VMI) et systèmes d'éclairage/signalisation à forte variété (Jokon), et quelles couches data/IA soutiennent la montée en gamme et le build-up Europe / Amérique du Nord annoncés ?"
...
Contexte : dépôt achard-arnaud/Hivest-ptf-analysis. Entité VMI-Jokon (entity_id vmi-jokon), cutoff 23/09/2026, mode FULL.
...
1. Lis AGENTS.md, program/state.json, companies/vmi-jokon/manifest.json. Ne relis pas le handoff programme entier.
...
3. Intègre la carte : crée ou complète companies/vmi-jokon/research_graph.json (nœuds question avec branch_6m, owner, trigger, budget_queries, stop_condition) et companies/vmi-jokon/INDEX.md ; reporte les lignes COO/DSI dans program/leadership_register.json avec promoter_signal et ownership_scope.
